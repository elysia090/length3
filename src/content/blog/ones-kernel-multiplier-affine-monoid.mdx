...
`ranks8` が 2 命令、`select8` が 3 命令で出た。そこから自明に落ちるもの — compaction、filtering、touched list、sparse offset、stable partition、Morton code、radix sort の 1 pass — は、わざわざ書いても面白くない。全部「mask から address を作って動かす」の言い換えである。
...
### 代入 1: af_pow を popcount 回に

combine は `k^(l₂)` を要求する。素朴には l₂ 回の乗算。

`pw2[i] = k^(2^i)` を 64 個 precompute すると、`k^n` は `popcount(n)` 回で済む。そして木が揃っていて n が 2 冪なら 1 回。

最初は level 表（`k^(8·2^d)` を段ごとに）を持つつもりだったが、pw2 があれば要らなかった。
