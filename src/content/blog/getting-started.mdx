...
The site is deployed to Cloudflare's global network via [Wrangler](https://developers.cloudflare.com/workers/wrangler/) as a named Worker (`length3`) — there is no Cloudflare Pages project.
...
Deployment runs against that generated config directly:
...
- **`deploy.yml`** — manually triggered via `workflow_dispatch`: `astro build` → `wrangler deploy --config dist/server/wrangler.json`. Deployment credentials are injected as repository secrets (`CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`). Manual dispatch keeps production deploys deliberate and decoupled from CI merges.
